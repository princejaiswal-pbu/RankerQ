<template>
  <div class="flex-1 flex flex-col bg-ca-50 min-h-screen">
    ...
    <div class="px-6 pt-6 pb-4 bg-white border-b border-[#E5F7E9]">
      ...
      <p class="mt-3 text-[11px] font-bold tracking-widest uppercase text-gray-400">Step 1 of 2 • Personalization</p>
    </div>
    ...
    <div class="flex-1 px-6 py-8 overflow-y-auto">
      ...
      <p class="mt-2 text-[14px] text-gray-500 leading-5">Let's personalize your journey. We’ll show quizzes & doubts relevant to your level.</p>
      ...
      <div class="mt-8 ca-card p-5">
        ...
        <div class="mt-2 relative">
          ...
          <!-- Dropdown -->
          <div v-if="showLevel" class="absolute z-10 mt-2 w-full bg-white rounded-xl border border-[#E5F7E9] shadow-xl overflow-hidden animate-in">
        ...
        <div v-if="form.level" class="mt-4 p-3 rounded-xl bg-ca-50 border border-ca-100 flex gap-2">
          <span class="text-ca-600 text-sm">ℹ️</span>
          <p class="text-[12px] text-ca-800 leading-4">{{ levelInfo }}</p>
        </div>
      </div>

      <button @click="submit" :disabled="!canContinue"
        :class="canContinue ? 'bg-ca-600 shadow-green text-white' : 'bg-gray-200 text-gray-400'"
        class="mt-6 w-full h-[52px] rounded-[14px] font-semibold jakarta text-[15px] transition active:scale-[0.98]">
        Continue to Dashboard →
      </button>
      <p class="mt-3 text-center text-[11px] text-gray-400">Telegram data is auto-secured • No spam</p>
    </div>
  </div>
</template>
...
function submit() { if (canContinue.value) emit('complete', { ...form }) }
</script>
