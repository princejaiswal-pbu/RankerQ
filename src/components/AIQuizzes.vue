<template>
  <div class="p-5 pb-24">
    <div class="flex justify-between items-center">
      ...
      <span class="text-[11px] font-bold px-2.5 py-1 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white">Gemini AI</span>
    </div>
      ...
      <div class="rounded-[19px] bg-white p-5">
        <div class="flex gap-3">
          ...
          <div>
            ...
            <p class="text-[12px] text-gray-500 mt-1">Fresh quizzes generated by Gemini for your chosen topic</p>
          </div>
        </div>
        ...
        <div class="mt-5 space-y-4">
          <div>
            ...
            <div class="mt-2 flex gap-2 overflow-x-auto scrollbar-hide pb-1">
              <button v-for="s in subjects" :key="s" @click="form.subject = s; form.chapter = ''" :class="form.subject === s ? 'bg-blue-600 text-white shadow' : 'bg-white border border-[#E5F7E9] text-gray-600'" class="whitespace-nowrap px-4 py-2 rounded-full text-[12px] font-semibold transition">{{ s }}</button>
            </div>
          </div>

          <div>
            <label class="text-[11px] font-bold tracking-widest uppercase text-gray-400">Chapter</label>
            <input v-model.trim="form.chapter" placeholder="e.g. Contract Act: consideration" class="mt-2 w-full h-11 px-4 rounded-xl bg-[#F9FFFB] border border-[#D1FAE5] text-[13px] font-medium outline-none focus:border-blue-500" />
          </div>
          ...
          <button @click="generateQuiz" :disabled="generating" :class="generating ? 'bg-gray-200 text-gray-400' : 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-[0_8px_20px_rgba(37,99,235,0.3)]'" class="w-full h-[48px] rounded-xl font-semibold text-[14px] flex items-center justify-center gap-2 transition active:scale-[0.98]">
            ...
            {{ generating ? 'Gemini is creating...' : `✨ Generate ${form.count}Q Quiz with Gemini` }}
          </button>
          <p class="text-[10px] text-gray-400 text-center">Powered by Gemini • Fresh questions every time</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="activeQuiz" class="mt-5">
      ...
      <div class="mt-5 ca-card p-5">
        ...
        <div v-if="answered" class="mt-4 p-4 rounded-xl bg-[#F9FFFB] border border-[#E5F7E9]">
          <p class="text-[11px] font-bold tracking-widest uppercase text-gray-400">Gemini Explanation</p>
...
async function generateQuiz() {
  ...
  try {
    const response = await fetch('/api/ai/generate-quiz', {
      ...
      body: JSON.stringify({ subject: form.subject, topic: form.chapter, difficulty: form.difficulty, count: form.count, level: store.level.value, mistakes: store.mistakes.value, apiKey: store.geminiKey.value })
    })
...
function shareResult() {
  const text = `🤖 I scored ${score.value}/${activeQuiz.value.questions.length} in AI Quiz (${form.subject} - ${form.chapter}) by Gemini! ${earnedXP.value} XP earned. Try: https://t.me/yourbot`
