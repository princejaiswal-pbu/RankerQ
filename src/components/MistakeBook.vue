<template>
  <div class="p-5 pb-24">
    <div class="flex justify-between items-center"><h2 class="jakarta text-[18px] font-bold">Saved Questions</h2><span class="text-[11px] font-bold px-2.5 py-1 rounded-full bg-red-50 text-red-600 border border-red-100">{{ mistakes.length }} saved</span></div>
    <div v-if="mistakes.length>0" class="mt-5 rounded-[20px] bg-gradient-to-br from-red-500 to-orange-500 p-[1px]"><div class="rounded-[19px] bg-white p-5"><div class="flex justify-between"><div><p class="text-[11px] font-bold tracking-widest uppercase text-red-600">Auto-Saved</p><h3 class="jakarta font-bold text-[18px] mt-1">You got {{ mistakes.length }} saved</h3><p class="text-[13px] text-gray-600 mt-1">Revise in {{ revisionTime }} mins • ICAI repeats 60%</p></div><div class="w-14 h-14 rounded-2xl bg-red-50 flex items-center justify-center text-2xl">📚</div></div><div class="mt-4 grid grid-cols-2 gap-2"><button @click="reviseAll" class="h-11 rounded-xl bg-gray-900 text-white font-semibold text-[13px]">Revise All →</button><button @click="clearAll" class="h-11 rounded-xl bg-white border text-[13px]">Clear mastered</button></div></div></div>
    <div v-else class="mt-10 text-center"><div class="w-20 h-20 rounded-[20px] bg-ca-50 border flex items-center justify-center mx-auto text-3xl">✨</div><h3 class="jakarta font-bold text-[16px] mt-4">No mistakes yet!</h3><p class="text-[13px] text-gray-500 mt-1">Bookmark questions or answer incorrectly to save them here.</p></div>
...
function reviseAll(){ alert(`Revising ${mistakes.value.length} saved questions`) }
</script>
