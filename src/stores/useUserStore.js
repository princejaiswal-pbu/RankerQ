...
const notifications = ref([
  { id: 1, title: 'New Quest Live! 🎮', desc: 'Business Laws quest live. +150 XP', time: '2h ago', read: false, icon: '🎯' },
  { id: 2, title: 'Streak Shield!', desc: 'Complete quest today to keep 7-day streak!', time: '5h ago', read: false, icon: '🔥' },
])
...
function saveItem(key,value){ const str=typeof value==='string'?value:JSON.stringify(value); localStorage.setItem(key,str); const c=getCloudStorage(); if(c) c.setItem(key,str,()=>{}) }
function getItemLocal(key,isJson=false){ const v=localStorage.getItem(key); if(!v) return null; if(isJson){ try{ return JSON.parse(v) }catch{ return null } } return v }
function calculateStreak(){
  ...
  if(last===yesterday){ streak.value=(parseInt(getItemLocal('rq_streak')||'0')||streak.value)+1; lastLogin.value=today; saveItem('rq_streak',String(streak.value)); saveItem('rq_last_login',today) }
  else{ streak.value=1; lastLogin.value=today; saveItem('rq_streak','1'); saveItem('rq_last_login',today) }
}
function addMistake(q){ if(mistakes.value.find(m=>m.id===q.id)) return; mistakes.value.unshift({ ...q, addedAt:Date.now(), revised:false }); saveItem('rq_mistakes',mistakes.value) }
function removeMistake(id){ mistakes.value=mistakes.value.filter(m=>m.id!==id); saveItem('rq_mistakes',mistakes.value) }
function addBookmark(q){ if(bookmarks.value.find(b=>b.id===q.id)) return; bookmarks.value.unshift({ ...q, bookmarkedAt:Date.now() }); saveItem('rq_bookmarks',bookmarks.value) }
function removeBookmark(id){ bookmarks.value=bookmarks.value.filter(b=>b.id!==id); saveItem('rq_bookmarks',bookmarks.value) }
function toggleBookmark(q){ if(bookmarks.value.find(b=>b.id===q.id)) removeBookmark(q.id); else addBookmark(q) }
...
export async function saveUserToTelegramStorage(userName,userLevel,apiKey=''){
  ...
  localStorage.setItem('rq_name',userName); localStorage.setItem('rq_level',userLevel); localStorage.setItem('rq_api_key',apiKey||geminiApiKey.value)
  localStorage.setItem('rq_user',JSON.stringify(data)); localStorage.setItem('rq_streak',String(streak.value)); localStorage.setItem('rq_quizzes',String(quizzesDone.value)); localStorage.setItem('rq_xp',String(xp.value)); localStorage.setItem('rq_mistakes',JSON.stringify(mistakes.value)); localStorage.setItem('rq_bookmarks',JSON.stringify(bookmarks.value))
  const cloud=getCloudStorage(); if(!cloud) return true
  return new Promise(res=>{ cloud.setItem('rq_name',userName,()=>{}); cloud.setItem('rq_level',userLevel,()=>{}); cloud.setItem('rq_api_key',apiKey||geminiApiKey.value,()=>{}); cloud.setItem('rq_user',JSON.stringify(data),(err)=>{ res(!err) }) })
}
...
export async function loadUserFromTelegramStorage(){
  ...
  return new Promise(res=>{
    cloud.getItems(['rq_name','rq_level','rq_api_key','rq_streak','rq_last_login','rq_quizzes','rq_xp','rq_mistakes','rq_bookmarks'],(err,values)=>{
      ...
      streak.value=parseInt(values.rq_streak||localStreak||'1'); quizzesDone.value=parseInt(values.rq_quizzes||localQuizzes||'0'); xp.value=parseInt(values.rq_xp||localXp||'0'); lastLogin.value=values.rq_last_login||''
...
export async function clearUserStorage(){
  const keys=['rq_name','rq_level','rq_api_key','rq_user','rq_streak','rq_last_login','rq_quizzes','rq_xp','rq_mistakes','rq_bookmarks','ca_name','ca_level','ca_user','ca_streak','ca_last_login','ca_quizzes','ca_xp','ca_mistakes']
  ...
  const cloud=getCloudStorage(); if(cloud) cloud.removeItems(keys,()=>{})
}
...
export function useUserStore(){
  ...
  function completeQuiz(xpEarned=150,wrongAnswers=[]){ quizzesDone.value+=1; xp.value+=xpEarned; wrongAnswers.forEach(q=>addMistake(q)); calculateStreak(); saveItem('rq_quizzes',String(quizzesDone.value)); saveItem('rq_xp',String(xp.value)); saveUserToTelegramStorage(name.value,level.value,geminiApiKey.value) }
